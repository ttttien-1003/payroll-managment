 ...
 */
void WelfareManager::calculateAllWelfareForEmployee(const IEmployee& employee, double& outBonuses, double& outAllowances, double& outDeductions) const {
    ...
    for (const auto& welfare : _welfareList) {
        ...
        if (welfare && welfare->isEligible(employee)) {
