 ...
 */
bool TransportationWelfare::isEligible(const IEmployee& employee, const AttendanceManager& attendanceManager, int month, int year) const {
    // Với TransportationWelfare, có thể mọi nhân viên đều đủ điều kiện
    // hoặc điều kiện không phụ thuộc vào tháng/năm/chấm công cụ thể.
    // Nếu không dùng month, year, attendanceManager thì vẫn phải khai báo để khớp lớp cha.
    // (void)attendanceManager; // Tránh warning unused parameter nếu không dùng
    // (void)month;
    // (void)year;
    return true; // Giả sử mọi nhân viên đều được
}